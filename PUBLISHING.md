...
Each pushed tag triggers `.github/workflows/release.yml`, which creates the
matching GitHub Release automatically — title `<package> v<version>`, notes
pulled from that version's own `## <version>` section of the package's
`CHANGELOG.md`. Nothing to run by hand; give it a minute, then check the
[Releases page](https://github.com/iamv4g/keyed_form/releases). Every
release is created with `--latest=false` — six independently-versioned
packages share one repo-wide "Latest" pill, so it can never correctly
describe more than one of them; the [Published versions](#published-versions)
table above is the real source of truth for what's current per package.
...
below is followed) — the workflow fails loudly (and skips creating a
release) if that section is missing, rather than publishing an empty one.

If the workflow doesn't fire, or a release needs recreating, the equivalent
manual command is:

```bash
pkg=keyed_form_gen; version=$(grep '^version:' "packages/$pkg/pubspec.yaml" | awk '{print $2}')
notes=$(awk -v v="$version" '
  $0 == "## " v { flag=1; next }
  /^## /        { flag=0 }
  flag
' "packages/$pkg/CHANGELOG.md")
gh release create "$pkg-v$version" --title "$pkg v$version" --notes "$notes" --latest=false
```
